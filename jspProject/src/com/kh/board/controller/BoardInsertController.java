...
public class BoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * form에서 multipart/form-data 형식으로 전송했기 때문에 기존 request의 parameter영역에서
		 * 꺼낼수가 없다. 특정 라이브러리를 사용하여 전달받아야 함
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 */
			//경로 마지막에 / 붙여주기 (그 폴더안에 저장할 것이기 때문에 /를 붙이면 더 편해짐)
...
//			System.out.println(savePath);
			 ...
			 * 
			 * 매개변수 생성자로 생성
			 * MultipartRequest multiRequest = new MultipartRequest(request객체,저장할 폴더경로,용량제한,인코딩값,파일명 수정객체);
			 * 
			 * 위 구문 한줄이 실행되는 순간 지정한 경로에 파일이 업로드 된다.
			 * 사용자가 올린 파일명은 그대로 해당 폴더에 업로드 하지 않는게 일반적이다. - 같은 파일명이 있을 경우 덮어씌워질 경우도 있고
			 * 한글/특수문자가 포함된 파일명 경우는 업로드되는 서버에따라 오류를 발생시킬 여지가 있기 때문.
			 * 
...
//			System.out.println(b);
			...
			//첨부파일이 있을 경우 원본명을 반환, 없는 경우는 null을 반환
			...
			if(result>0) {
				...
				if(at!=null) { //넘어온파일이 있어서 객체가 생성되었다면
