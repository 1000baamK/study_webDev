...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1)POST요청일 경우 전달값에 한글이 포함되어 있다면 encoding설정을 해야한다.
		 ...
		 * 3)request : request에 담은 데이터 해당 request를 포워딩한 응답jsp에서만 꺼내쓸수 있다.
		 * 4)page : 해당 jsp 페이지에서만 꺼내쓸수 있음.
		 * 
		 * 공통적으로 데이터를 담고자 한다면
		 * setAttribute("키","벨류");
		 * 
		...
		if(loginUser==null) { //로그인 실패(에러 페이지로 응답해보기)
			...
			// view.forward(request, response);
			//2.재요청(리다이렉트) - 새 페이지를 보여줘(url에 기존 주소가 남아있지 않음)
